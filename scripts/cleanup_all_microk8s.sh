...
run kubectl delete -f k8s/frontend/frontend.yaml -n "$NAMESPACE" --ignore-not-found
run kubectl delete -f k8s/backend/backend.yaml -n "$NAMESPACE" --ignore-not-found

# 3) Delete CouchDB StatefulSet, Deployments & Services (by manifest and by label)
# Delete manifest-driven resources (if present)
run kubectl delete -f k8s/database/couchdb-statefulset.yaml -n "$NAMESPACE" --ignore-not-found
#run kubectl delete -f k8s/database/couchdb-service.yaml -n "$NAMESPACE" --ignore-not-found
#run kubectl delete -f k8s/database/couchdb-headless.yaml -n "$NAMESPACE" --ignore-not-found
run kubectl delete -f k8s/basic-database/couchdb.yaml -n "$NAMESPACE" --ignore-not-found || true
...
run kubectl delete -f k8s/PostStage/shisha-sample-data.yaml -n "$NAMESPACE" --ignore-not-found
...
run kubectl delete -f k8s/hpa/hpa-frontend.yaml -n "$NAMESPACE"  --ignore-not-found || true
run kubectl delete -f k8s/hpa/hpa-backend.yaml -n "$NAMESPACE"  --ignore-not-found || true
run kubectl -n "$NAMESPACE" delete hpa --all --ignore-not-found || true

run kubectl delete -f k8s/pdb/pdb-backend.yaml -n "$NAMESPACE"  --ignore-not-found || true
run kubectl delete -f k8s/pdb/pdb-frontend.yaml -n "$NAMESPACE"  --ignore-not-found || true
...
run kubectl delete -f k8s/frontend/ingress.yaml -n "$NAMESPACE" --ignore-not-found || true
run kubectl delete -f k8s/PreStage/ingress-treafik.yaml --ignore-not-found || true
